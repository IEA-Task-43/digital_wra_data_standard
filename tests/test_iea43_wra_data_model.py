...
def get_demo_data_files():
    return [os.path.join(DEMO_DATA_DIR, file_name) for file_name in os.listdir(DEMO_DATA_DIR) if file_name.endswith(".json")]

@pytest.mark.parametrize("file_path", get_demo_data_files())
def test_iea43_wra_data_model(file_path):
